...
if (!defined('INST_BASEDIR')) {
    ...
    */
    ...
    $parts = [$special, $serial, $connect, $admin];
    ...
    if (isset($_POST) && count($_POST) > 0) {
            ...
            foreach ($steps[STEP_ADDEDINFO]['form'] as $control) {
                if (is_array($control) && isset($control['keyword'])) {
                    if (isset($control['value_off'])) {
                        ...
                        $_SESSION[$ID][STEP_ADDEDINFO][$control['keyword']] = '';
                    }
                }
            }
        ...
        // For the reset of the installer
            ...
            foreach ($_POST as $key => $value) {
                ...
                elseif (array_key_exists($key, $_SESSION[$ID][$connect])) {
                    // If encryption is enabled, encrypt the login to session
                    if ($steps[STEP_DBCONNECT]['encryptlogin'] && in_array($key, ['password', 'username', 'hostname', 'database'])) {
                ...
                elseif (array_key_exists($key, $_SESSION[$ID][$admin])) {
                    if ($steps[STEP_ROOTUSER]['encryptdata']) {
    ...
    if (!isset($_SESSION[$ID][$stepkey])) {
        ...
        */
        ...
        if ($steps[$firstStep]['enabled'] && !$firstIsHidden) {
            ...
            $_SESSION[$ID][$stepkey] = GetNextStep($firstStep);
        }
    }
