...
if (!defined('INST_BASEDIR')) {
    ...
    *  ================================================================= */
    ...
    function SetDatabaseInstallStatus($database, $state, $prefix = '', $successCount = 0, $failedCount = 0)
    {
        ...
        $_SESSION[$ID][$sql][$database] = ['state' => $state, 'prefix' => $prefix, 'okaycount' => $successCount, 'failcount' => $failedCount];
    }
    ...
    function GetDatabaseInstallStatus($database = '')
    {
        ...
        elseif (array_key_exists($database, $_SESSION[$ID][$sql])) {
  ...
  $LICENSE_SERV = 'http://flare.miscy.net/server/check.php?key=';
    ...
    function TestFolderWriteability($folderPath = '')
    {
        ...
        if (strlen($folderPath) == 0) {
            ...
            if (is_writable($folderPath)) {
                ...
                return TestResults('failed', 'Config folder <b>'.$folderPath.'</b> is not writeable');
            }
        } else {
            if (mkdir($folderPath)) {
                if (is_writable($folderPath)) {
                    ...
                    return TestResults('failed', 'Config folder <b>'.$folderPath.'</b> has been created successfully, but it is not writeable');
                }
            } else {
    ...
    function IsInstallerDone()
    {
        ...
        foreach ($steps[STEP_WRITECONFIG]['configs'] as $file) {
            ...
            if (is_dir($savepath) || strlen($savepath) == 0) {
                ...
                elseif (is_file($savepath) && !$updateOnZero) {
                    continue;
                }
    ...
    function CompareVersons($versionA, $versionB)
    {
        if ($versionA[0] < $versionB[0]) {
            return -1;
        } elseif ($versionA[0] == $versionB[0]) {
            if ($versionA[1] < $versionB[1]) {
                return -1;
            } elseif ($versionA[1] == $versionB[1]) {
                if ($versionA[2] < $versionB[2]) {
                    return -1;
                } elseif ($versionA[2] == $versionB[2]) {
                    return 0;
                } elseif ($versionA[2] > $versionB[2]) {
                    return 1;
                }
