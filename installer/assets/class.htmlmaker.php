<?php if(!defined('INST_BASEDIR')) die('Direct access is not allowed!');
    ...
    *  ================================================================= */
    ...
    class Inst_HtmlMaker
    {
		 ...
		 */
		function HideIsHiddenSteps($hideValue)
		{
		 ...
		 */
        function FormTextarea($value, $name='input', $optional=array(), $containerAddClass='')
        {
            $attribs = $this->GetAttributeString($optional);
			$containerAddClass = (strlen($containerAddClass) > 0) ? ' '.$containerAddClass : '';
            $this->queue[] = 
		 ...
		 */
		function UpdateQueueAtIndex($index, $item)
		{
...
<?php 
		...
		foreach($steps as $key=>$step)
		{
			...
			if($key == STEP_SETTOVERVIEW)
			{
				...
				else echo '<li class="settings"><a style="btn btn-primary" href="?step='.$key.'">'.$step['title'].'</a></li>'."\n";
				continue;
			}
