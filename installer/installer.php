...
if (!defined('INST_BASEDIR')) {
    die('Installer cannot be started directly!');
}
    ...
    *  ================================================================= */
    ...
    // If the installer WILL display something
        ...
        if (IsInstallerDone() == false) {
            ...
            if ($steps[STEP_WELCOME]['enabled'] && $step == STEP_WELCOME) {
                ...
                $page->FormStart(['step' => GetNextStep(STEP_WELCOME)]);

                $prev = GetPrevStep(STEP_WELCOME);
                if ($prev) {
                    $page->FormButton('Back', ['step' => $prev]);
                }
            ...
            if ($steps[STEP_PHPREQUIRES]['enabled']) {
                ...
                if ($steps[STEP_PHPREQUIRES]['phpversion'] !== false) {
                    ...
                    $page->StartTable(4, ['class' => 'dbtests', 'cellspacing' => '0', 'cellpadding' => '0']);

                    // Display the requirements
                    $page->AddTableData('', ['class' => 'currentico']);
                    $page->AddTableData('Required:', ['style' => 'padding-right:15px;']);
                    ...
                    $page->AddTableData(implode('.', $bounds['lower']).$str, ['style' => 'padding-right:15px;']);
                    $page->AddTableData('');

                    // If current version is equal or higher than lower bound
                    // and is below the upper bound - then accepted!
                    if ($CurLow >= 0 && $CurHig < 0) {
                        $page->AddTableData('', ['class' => 'okayico']);
                        $page->AddTableData('You have:');
                        $page->AddTableData(phpversion(), ['style' => 'text-align:center;']);
                        $page->AddTableData('Supported!', ['class' => 'okay']);
                    } else {
                        $phpversion = false;
                        if ($CurLow < 0) {
                        ...
                        $page->AddTableData('', ['class' => 'failico']);
                        $page->AddTableData('You have:');
                        $page->AddTableData(phpversion(), ['style' => 'text-align:center;']);
                        $page->AddTableData($str, ['class' => 'fail']);
                    }
                    $page->EndTable();
                } // End Version check
                ...
                if (count($steps[STEP_PHPREQUIRES]['extensions']) > 0) {
                    ...
                    $page->StartTable(3, ['class' => 'dbtests', 'cellspacing' => '0', 'cellpadding' => '0']);
                    foreach ($steps[STEP_PHPREQUIRES]['extensions'] as $extKey => $extTitle) {
                        if (IsExtensionInstalled($extKey)) {
                            $page->AddTableData('', ['class' => 'okayico']);
                            $page->AddTableData('<strong title="'.$extKey.'">'.$extTitle.'</strong>', ['style' => 'padding-right:15px;']);
                            $page->AddTableData('Installed!', ['class' => 'okay']);
                        } else {
                            $extensions = false;
                            $page->AddTableData('', ['class' => 'failico']);
                            $page->AddTableData('<strong title="'.$extKey.'">'.$extTitle.'</strong>', ['style' => 'padding-right:15px;']);
                            $page->AddTableData('Not Installed!', ['class' => 'fail']);
                        }
                    }
                    $page->EndTable();
                ...
                if ($steps[STEP_PHPREQUIRES]['directives'] && count($php_directives) > 0) {
                    ...
                    $page->StartTable(4, ['class' => 'dbtests']);
                    $page->AddTableData('', [], ['style' => 'color:#838383;']);
                    $page->AddTableData('Directive Title');
                    $page->AddTableData('Required', ['style' => 'padding:0 9px 0 9px;']);
                    $page->AddTableData('Current', ['style' => 'padding:0 9px 0 9px;']);
                    ...
                    foreach ($php_directives as $idx => $directive) {
                        ...
                        elseif (isset($directive['orhigher']) || isset($directive['orlower'])) {
                            ...
                            } elseif (isset($directive['orlower'])) {
                        ...
                        $page->AddTableData('', ['class' => $icon]);
                        $page->AddTableData('<strong title="'.$directive['inikey'].'">'.$directive['title'].'</strong>',
                            ['style' => 'padding-right:12px;']);
                        $page->AddTableData('<tt>'.$required.'</tt>', ['style' => 'text-align:center;']);
                        $page->AddTableData('<tt>'.$current.'</tt>', ['style' => 'text-align:center;']);
                    }
                    $page->EndTable();
                ...
                if ($phpversion && $extensions && $directives) {
                    $page->FormStart(['step' => GetNextStep(STEP_PHPREQUIRES)]);
                ...
                // file IO - show "retry" button
                    ...
                    $page->FormStart(['step' => STEP_PHPREQUIRES]);
                    $prev = GetPrevStep(STEP_PHPREQUIRES);
                    if ($prev) {
                        $page->FormButton('Back', ['step' => $prev]);
                    }
            ...
            if ($steps[STEP_IOFILES]['enabled']) {
                ...
                foreach ($results as $testTypes) {
                    foreach ($testTypes as $test => $result) {
                        if ($result['state'] == 'success') {
                            $page->AddTableData('', ['class' => 'okayico']);
                            $page->AddTableData($test, ['class' => 'operation']);
                            $page->AddTableData('Success!', ['class' => 'okay']);
                            $page->AddTableData($result['msg'], ['class' => 'normalmsg']);
                        } elseif ($result['state'] == 'failed') {
                            $page->AddTableData('', ['class' => 'failico']);
                            $page->AddTableData($test, ['class' => 'operation']);
                            $page->AddTableData('Failed!', ['class' => 'fail']);
                            $page->AddTableData($result['msg'], ['class' => 'errormsg']);
                        } elseif ($result['state'] == 'exists') {
                            $page->AddTableData('', ['class' => 'existico']);
                            $page->AddTableData($test, ['class' => 'operation']);
                            $page->AddTableData('Exists', ['class' => 'exist']);
                            $page->AddTableData($result['msg'], ['class' => 'normalmsg']);
                        } else {
                            $page->AddTableData('', ['class' => 'unknownico']);
                            $page->AddTableData($test, ['class' => 'operation']);
                            $page->AddTableData('Not tested', ['class' => 'unknown']);
                            $page->AddTableData($result['msg'], ['class' => 'unknownmsg']);
                        }
                    }
                }
                $page->EndTable();
                ...
                if ($ioable) {
                    $page->FormStart(['step' => GetNextStep(STEP_IOFILES)]);
                ...
                // file IO - show "retry" button
                    ...
                    $page->FormStart(['step' => STEP_IOFILES]);
            ...
            if ($steps[STEP_SERIALKEY]['enabled'] && ($showSerialForm || $step == STEP_SERIALKEY)) {
                ...
                $page->Paragraph('Please enter purchased serial key for <strong>easySite v'.
                                 file_get_contents('latest').'</strong> below.');

                $page->Label('Serial key:');
                $page->FormStart(['step' => GetNextStep(STEP_SERIALKEY)]);
                ...
                if ($steps[STEP_SERIALKEY]['allowtrial']) {
                    ...
                    $page->FormStart(['step' => GetNextStep(STEP_SERIALKEY), 'isTrial' => 'selected']);
                    ...
                    $page->Paragraph('You can enjoy the free version Do you '.
                                     'want to try it out first?');

                    $page->FormSubmit('Try it out ');
                    $page->FormClose();
                }
            ...
            if ($steps[STEP_LANGUAGE]['enabled'] && count($steps[STEP_LANGUAGE]['supported']) > 0 && $step == STEP_LANGUAGE) {
                ...
                $page->FormStart(['step' => GetNextStep(STEP_LANGUAGE)]);
            ...
            */
            ...
            if ($steps[STEP_TIMEZONE]['enabled'] && $step == STEP_TIMEZONE) {
                ...
                $page->FormStart(['step' => GetNextStep(STEP_TIMEZONE)]);
            ...
            if ($steps[STEP_ADDEDINFO]['enabled']) {
                ...
                $page->FormStart(['step' => GetNextStep(STEP_ADDEDINFO)]);
                    ...
                    switch ($key) {
                        case 'checkbox':
                            if (HasAnyAdditionalStepValueBeenPostedYet()) {
                                $checked = (GetAdditionalControlValue($data['keyword'], $data['value_off']) == $data['value_on']) ? ['checked'] : [];
                            } else {
                        ...
                        case 'radiobox':
                            if (HasAnyAdditionalStepValueBeenPostedYet()) {
                                $checked = (GetAdditionalControlValue($data['keyword'], '') == $data['value']) ? ['checked'] : [];
                            } else {
                                $checked = (isset($data['checked']) && $data['checked'] === true) ? ['checked'] : [];
                            }
            ...
            if ($steps[STEP_DBCONNECT]['enabled']) {
                ...
                if (!$dbase->IsConnected() || $step == STEP_DBCONNECT) {
                    ...
                    elseif ($step == STEP_DBCONNECT && $dbase->IsConnected()) {
                    ...
                    if ($steps[STEP_DBCONNECT]['encryptlogin'] && $dbase->IsConnected()) {
                        $page->FormStart(['step' => GetNextStep(STEP_DBCONNECT)]);
                        ...
                        $page->FormStart(['step' => STEP_DBCONNECT, 'reset' => 'connection']);
                    ...
                    // Only shown when not connected to database server
                        ...
                        if ($steps[STEP_DBCONNECT]['portoptional']) {
                            $page->StartTable(2, ['class' => 'hostport', 'cellpadding' => '0', 'cellspacing' => '0']);

                            // Insert the elements in wrong order, then when the
                            // items are popped they are inserted in right order
                            $page->FormInput($login['hostname'], 'hostname', [], 'bigbox');
                            $page->Label('Hostname:');
                            $page->AddTableData($page->PopQueue().$page->PopQueue(), ['class' => 'port']);

                            $page->FormInput($login['dbport'], 'dbport', [], 'tinybox');
                            $page->Label('Port:');
                            $page->AddTableData($page->PopQueue().$page->PopQueue(), ['class' => 'host']);
                        ...
                        if ($steps[STEP_DBCONNECT]['encryptlogin']) {
                            ...
                            $page->FormInput($login['password'], 'password');
                        }
            ...
            if ($steps[STEP_DBSELECT]['enabled']) {
                ...
                if (!$dbase->IsDatabaseSelected() || $step == STEP_DBSELECT) {
                    ...
                    if ($step != STEP_DBSELECT) {
                        ...
                        } elseif (!$dbase->IsDatabaseSelected()) {
                    ...
                    elseif ($steps[STEP_DBSELECT]['selecttype'] == 'dblist' || $steps[STEP_DBSELECT]['selecttype'] == 'both') {
                    ...
                    if ($steps[STEP_DBSELECT]['allowcreate']) {
                        ...
                        $page->FormStart(['step' => STEP_DBSELECT], ['#' => 'newdb']);
                        $page->Label('Database name:');
                        $page->FormInput($newdb, 'createdb', [], 'mediumbox');
            ...
            if ($steps[STEP_DBACCESS]['enabled']) {
                ...
                if (!$dbtest['totalsuccess'] || (!$steps[STEP_DBACCESS]['ishidden'] && $step == STEP_DBACCESS)) {
                    ...
                    $page->StartTable($columnCount, ['class' => 'dbtests', 'cellspacing' => '0', 'cellpadding' => '0']);
                    foreach ($dbtest as $operation => $result) {
                        ...
                        if ($result['success']) {
                            $page->AddTableData('', ['class' => 'okayico']);
                            $page->AddTableData($operation, ['class' => 'operation']);
                            $page->AddTableData('Success!', ['class' => 'okay']);
                            ...
                            if ($config['show_database_error_messages']) {
                                $page->AddTableData($result['error'], ['class' => 'errormsg']);
                            }
                        }
                    }
                    $page->EndTable();

                    $page->FormStart(['step' => GetNextStep(STEP_DBACCESS)]);
                    $page->FormButton('Back', ['step' => GetPrevStep(STEP_DBACCESS)]);
            ...
            if ($steps[STEP_DBPREFIX]['enabled']) {
                ...
                if ($validPrefix == false || $step == STEP_DBPREFIX) {
                    ...
                    if ($step != STEP_DBPREFIX) {
                        ...
                        } elseif ($dbase->IsDatabaseFriendly($prefix) == false) {
                    ...
                    // the first time and no error should be shown to him!
                        ...
                        elseif (strlen($prefix) == 0 && $steps[STEP_DBPREFIX]['optional']) {
                ...
                if (count($successInstall) == 0 || $step == STEP_RUNSQL) {
                    ...
                    *  step STEP_DBPREFIX is previous step from STEP_RUNSQL!  */
                    ...
                    if (count($tableList) > 0) {
                        ...
                        $page->StartTable(3, ['class' => 'tablelist']);
                    ...
                    $dbStatus = GetDatabaseInstallStatus($dbase->GetDatabaseName());
                    if ($dbStatus['state'] == 'success') {
                        $page->FormStart(['step' => GetNextStep(STEP_RUNSQL)]);
                        $page->FormButton('Back', ['step' => GetPrevStep(STEP_RUNSQL)]);
                        $page->FormSubmit('Next');
                        $page->FormClose();
                    }
                    ...
                    elseif ($dbStatus == false) {
                        $page->FormStart(['step' => STEP_RUNSQL, 'runsql' => $dbase->GetDatabaseName()]);
                        $page->FormButton('Back', ['step' => GetPrevStep(STEP_RUNSQL)]);
            ...
            if ($steps[STEP_ROOTUSER]['enabled']) {
                ...
                if (!AdminAccountSuccess() || $step == STEP_ROOTUSER) {
                        ...
                        **************************************************************************************/
                        ...
                        if ($validData) {
                            ...
                            if ($databaseFailedCount == 0) {
                                ...
                                $page->SuccessBox('A user account has been created successfully. Proceed to the next step.');
                            } else {
                    ...
                    elseif ($step == STEP_ROOTUSER && AdminAccountSuccess()) {
                    ...
                    if (!AdminAccountSuccess()) {
                        ...
                        $page->FormStart(['step' => STEP_ROOTUSER, 'create' => 'administrator']);
                        $page->InfoBox('Your passwords are stored in a very secure file using standard MD5 encryption.');
                        ...
                        $page->FormButton('Back', ['step' => GetPrevStep(STEP_ROOTUSER)]);
            ...
            if ($steps[STEP_WRITECONFIG]['enabled']) {
                ...
                if ($allCreated) {
                    $page->FormStart(['step' => GetNextStep(STEP_WRITECONFIG)]);
                ...
                // Offer retry if creation failed
                    ...
                    $page->FormStart(['step' => STEP_WRITECONFIG]);
                    $page->FormButton('Back', ['step' => GetPrevStep(STEP_WRITECONFIG)]);
                    $page->FormSubmit('Retry');
                    $page->FormClose();
                }
    ...
    function DatabaseManualBoxForm($boxLabelTitle = 'Type in database name manually:')
    {
        ...
        $page->FormStart(['step' => GetNextStep(STEP_DBSELECT)]);

        $page->Label($boxLabelTitle);
        $page->FormInput($login['database'], 'database', [], 'mediumbox');

        $page->FormButton('Back', ['step' => GetPrevStep(STEP_DBSELECT)]);
        $page->FormSubmit('Next');
        $page->FormClose();
    }
    ...
    function DatabaseSelectionListForm()
    {
        ...
        $page->FormStart(['step' => GetNextStep(STEP_DBSELECT)]);
        $page->StartTable(2, ['class' => 'dblist']);
    ...
    function TheLastInstallerStep()
    {
            ...
            if ($config['automatically_self_destruct'] ||
                ...
                *  been deleted. */
                ...
                if (is_array($deletionStatus)) {
                    ...
                    } elseif ($files['success'] == 0 && $files['total'] > 0) {
        ...
        */
        ...
        if ($config['allow_overriding_oldconfig']) {
            ...
            $page->FormStart(['doneaction' => 'removeold']);
        ...
        if ($config['allow_self_destruction']) {
            ...
            $page->FormStart(['doneaction' => 'selfdestruct']);
        ...
        if (!$config['allow_overriding_oldconfig'] && !$config['allow_self_destruction']) {
            ...
            session_destroy();
            file_put_contents('data/installed', '');
        }
